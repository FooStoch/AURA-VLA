...
def get_aura_vla_module(cfg: Any, llm_dim: int, patches_per_image: int, num_images: int) -> AURAVLAModule:
    """
    Load the optional AURA-VLA module for adaptive-uncertainty flow-matching inference.
    """
    aura_vla_module = AURAVLAModule(
        llm_dim=llm_dim,
        action_dim=ACTION_DIM,
        patches_per_image=patches_per_image,
        num_images=num_images,
        hidden_dim=getattr(cfg, "aura_hidden_dim", None),
        uncertainty_loss_weight=getattr(cfg, "aura_uncertainty_loss_weight", 1.0),
        grounding_loss_weight=getattr(cfg, "region_alignment_loss_weight", 1.0),
        grounding_dice_weight=getattr(cfg, "region_alignment_dice_weight", 1.0),
        contrastive_loss_weight=getattr(cfg, "region_contrastive_loss_weight", 1.0),
        equivariance_loss_weight=getattr(cfg, "region_equivariance_loss_weight", 1.0),
        uncertainty_error_scale=getattr(cfg, "aura_uncertainty_error_scale", 1.0),
        contrastive_temperature=getattr(cfg, "region_contrastive_temperature", 0.07),
    ).to(DEVICE)
    aura_vla_module = aura_vla_module.to(torch.bfloat16).to(DEVICE)
    aura_vla_module.eval()

    checkpoint_path = find_checkpoint_file(cfg.pretrained_checkpoint, "aura_vla_module")
    state_dict = load_component_state_dict(checkpoint_path)
    aura_vla_module.load_state_dict(state_dict)
    return aura_vla_module


def get_action_head(cfg: Any, llm_dim: int) -> Union[L1RegressionActionHead, DiffusionActionHead, FlowMatchingActionHead]:
    """
    ...
    enabled_action_modes = sum([cfg.use_l1_regression, cfg.use_diffusion, use_flow_matching])
    assert enabled_action_modes == 1, "Enable exactly one of L1 regression, diffusion, or flow matching action head."
    ...
    if cfg.use_l1_regression:
        ...
        action_head = FlowMatchingActionHead(
            ...
            hidden_dim=llm_dim,
            action_dim=ACTION_DIM,
            num_flow_steps_train=getattr(cfg, "num_flow_steps_train", 50),
            num_flow_steps_inference=getattr(cfg, "num_flow_steps_inference", 8),
        )
    else:
        raise ValueError("One action mode must be True")
...
def get_vla_action(
    ...
    aura_vla_module: Optional[torch.nn.Module] = None,
    ...
    Args:
        ...
        aura_vla_module: Optional AURA-VLA module for adaptive flow-matching inference
        uncertainty_threshold: Threshold for early flow-matching termination

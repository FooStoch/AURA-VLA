"""AURA-VLA modules for adaptive-uncertainty and region-aligned fine-tuning."""
...
class AURAVLAModule(nn.Module):
    """Combines adaptive uncertainty and region-alignment auxiliary losses."""
    ...
    def forward(
        ...
        if predicted_actions is not None and ground_truth_actions is not None:
            ...
            metrics["aura_uncertainty_loss"] = _as_float_metric(uncertainty_loss)
            metrics["aura_predicted_uncertainty"] = _as_float_metric(uncertainty.mean())
            metrics["aura_target_risk"] = _as_float_metric(risk_targets.mean())
        ...
        if projector_features is not None and target_masks is not None:
            ...
            metrics["region_alignment_bce_loss"] = _as_float_metric(bce_loss)
            metrics["region_alignment_dice_loss"] = _as_float_metric(dice_loss)
            metrics["region_alignment_loss"] = _as_float_metric(grounding_loss)
            ...
            if negative_target_masks is not None:
                ...
                metrics["region_contrastive_loss"] = _as_float_metric(contrastive_loss)
        ...
        if counterfactual_predicted_actions is not None and counterfactual_target_actions is not None:
            ...
            metrics["region_equivariance_loss"] = _as_float_metric(equivariance_loss)

        metrics["aura_vla_loss"] = _as_float_metric(loss)
        return AURAVLAOutput(
            loss=loss,
            metrics=metrics,
